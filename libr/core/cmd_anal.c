...
#define geti(x) r_config_get_i(core->config, x);
	...
	if (is_debug) {
		...
		r_list_foreach (list, iter, map) {
			...
			snprintf (out, sizeof(out), "from 0x%"PFMT64x" to 0x%"PFMT64x" (aav)", map->itv.addr, r_itv_end (map->itv));
			rowlog (core, out);
			rowlog_done (core);
		...
		r_list_foreach (list, iter2, map2) {
			...
			snprintf (out, sizeof(out), "Value from 0x%08"PFMT64x " to 0x%08" PFMT64x " (aav)", from, to);
			rowlog (core, out);
			rowlog_done (core);
			r_list_foreach (list, iter, map) {
				...
				snprintf (out, sizeof(out), "0x%08"PFMT64x"-0x%08"PFMT64x" in 0x%"PFMT64x"-0x%"PFMT64x" (aav)", from, to, begin, end);
				rowlog (core, out);
				rowlog_done (core);
